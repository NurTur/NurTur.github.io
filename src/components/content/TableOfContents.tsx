...
export default function TableOfContents({ toc, activeSection, minLevel }: TableOfContentsProps) {
  ...
  return (
    <div id="toc-container" className="hidden max-h-[calc(100vh-9rem-113px)] overflow-auto pb-4 lg:block">
      ...
      <div className="mt-4 flex flex-col space-y-2 text-base">
        {toc
          ? toc.map(({ id, level, text }) => (
              <TOCLink id={id} key={id} activeSection={activeSection} level={level} minLevel={minLevel} text={text} />
            ))
          : null}
      </div>
    </div>
