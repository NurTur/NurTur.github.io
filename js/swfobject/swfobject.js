...
var swfobject = (function () {
  ...
  var h = (function () {
    var v =
        typeof K.getElementById != b &&
        typeof K.getElementsByTagName != b &&
        typeof K.createElement != b,
      AC = [0, 0, 0],
      x = null;
    if (typeof T.plugins != b && typeof T.plugins[n] == Q) {
      ...
      if (
        x &&
        !(
          typeof T.mimeTypes != b &&
          T.mimeTypes[P] &&
          !T.mimeTypes[P].enabledPlugin
        )
      ) {
    ...
    var AD = T.userAgent.toLowerCase(),
      ...
      AA = /webkit/.test(AD)
        ? parseFloat(AD.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1"))
        : false,
      q = false,
      z = r ? /win/.test(r) : /win/.test(AD),
      w = r ? /mac/.test(r) : /mac/.test(AD);
    /*@cc_on q=true;@if(@_win32)z=true;@elif(@_mac)w=true;@end@*/ return {
      w3cdom: v,
      pv: AC,
      webkit: AA,
      ie: q,
    ...
    for (var q = 0; q < t; q++) {
      ...
      if (h.pv[0] > 0) {
        ...
        if (r) {
          ...
          N[q].height = r.getAttribute("height")
    ...
    if (h.win && h.ie) {
      ...
      if (r) {
        ...
        if (w) {
          ...
          for (var t = 0; t < q; t++) {
            if (
              !(w[t].nodeType == 1 && w[t].nodeName == "PARAM") &&
              !(w[t].nodeType == 8)
            ) {
